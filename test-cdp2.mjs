  ...
  async function drive(steerAngle, ticks, label) {
    ...
    for (let i = 0; i < ticks; i++) {
      ...
      if (Math.abs(steerAngle) > 5) {
        // Cap drag at ±20px to prevent pitch coupling from spherical projection
        const rawDrag = -steerAngle * 0.15;
        const dragPx = Math.round(Math.max(-20, Math.min(20, rawDrag)));
